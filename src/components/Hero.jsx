  ...
  return (
    ...
    >
      ...
      <div className="container mx-auto px-6 relative z-10">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 1, delay: 0.1 }} className="max-w-4xl">
        <motion.h1 
...
>
  {textArray[textIndex]}
          ...
          <motion.p initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.8 }} className="text-lg md:text-xl lg:text-2xl text-zinc-300 mb-8 max-w-3xl">
            We craft web experiences that don't just look good — they create business revolutions.
          </motion.p>
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 1, delay: 1 }} className="flex flex-col sm:flex-row gap-4">
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="relative px-8 py-4 bg-cyan-500 rounded-full text-lg font-semibold overflow-hidden group">
              <Link to="/#contact" className="relative z-10">Start Your Journey</Link>
            </motion.button>
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="relative px-8 py-4 border border-cyan-500/30 rounded-full text-lg bg-zinc-900/20 backdrop-blur-lg group" onClick={() => setShowModal(true)}>
